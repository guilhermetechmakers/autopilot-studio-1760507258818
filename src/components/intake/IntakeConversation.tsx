...
const Message: React.FC<MessageProps> = ({ type, content, timestamp, isTyping = false }) => {
  return (
    <div className={cn(
      ...
      type === "ai" ? "bg-muted/50" : "bg-primary/5"
    )}>
      <div className={cn(
        "flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center",
        type === "ai" ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"
      )}>
...
export const IntakeConversation: React.FC<IntakeConversationProps> = ({
  questions,
  onResponseSubmit,
  isLoading = false,
